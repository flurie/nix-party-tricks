...
\section{Preamble}
\label{sec:org903054a}
\begin{frame}[label={sec:orgaa312f9}]{Nix is magic}
Surprise, this is a magic show!

Nix is a special kind of magic for specifying pretty much any output you could want.
\end{frame}
\begin{frame}[label={sec:org1f14fd6}]{Nothing up my sleeve\ldots{}}
Behold, a fresh VM.

I am clicking through these things because I am very serious.
\end{frame}
\begin{frame}[label={sec:orgf17737f}]{\ldots{}but cards on the table}
There's always something to install when you start.
\end{frame}
\begin{frame}[label={sec:orge4b59a5}]{For those playing along at home}
...
\label{sec:org5095b94}
\begin{frame}[label={sec:org94efa90}]{Purpose}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org86008f2}]{Let's install nix!}
...
\end{exampleblock}
\end{frame}
\begin{frame}[label={sec:orgacb7c22},fragile]{And let's install some things to make our lives easier (and this demo shorter)}
...
\label{sec:orgbe4adcc}
\begin{frame}[label={sec:org4687a53}]{Nix the First: Language}
Main features:
\begin{itemize}
\item functional
\item dynamic
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orge797642}]{Nix the First: Language (con't)}
Quirky type system:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgb7091a4}]{Nix the Second: Package Manager}
nixpkgs
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org9279b4f}]{Nix the Second: Package Manager (con't)}
\begin{figure}[htbp]
\centering
...
\end{figure}
\end{frame}
\begin{frame}[label={sec:org4204c17}]{Nix the Third: Linux Distribution}
NixOS
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgf392474},fragile]{Nix the Fourth: \emph{misc} tooling}
...
\label{sec:org81cde78}
\begin{frame}[label={sec:orgecbcb57}]{Misc tools for environment management}
...
\end{alertblock}
\end{frame}
\begin{frame}[label={sec:orgee4d44e}]{Let's install direnv!}
...
\end{exampleblock}
\end{frame}
\begin{frame}[label={sec:org7ebe686},fragile]{Let's grab the code\ldots{}}
...
\end{exampleblock}
\end{frame}
\begin{frame}[label={sec:org4267960},fragile]{\ldots{}and then let the magic take hold}
...
\end{exampleblock}
\end{frame}
\begin{frame}[label={sec:orgdce95d8}]{Tour our new powers}
\begin{figure}[htbp]
\centering
...
\end{figure}
\end{frame}
\begin{frame}[label={sec:org8d07b29},fragile]{A note about creds}
...
\end{alertblock}
\end{frame}
\begin{frame}[label={sec:orge990d6f},fragile]{Enter AWS with train}
...
\end{exampleblock}
\end{frame}
\begin{frame}[label={sec:org99a8084},fragile]{Time to test the thing out}
...
\label{sec:orge78af6e}
\begin{frame}[label={sec:org3df2079},fragile]{Preamble}
...
\end{exampleblock}
\end{frame}
\begin{frame}[label={sec:org19ebb0b}]{Misc tools for deployment management}
\begin{alertblock}{Tools we will use in this section}
\begin{itemize}
\item cachix: arbitrary caching for nix derivations
...
\end{alertblock}
\end{frame}
\begin{frame}[label={sec:orgd99bf36}]{NixOS on AWS three ways}
\#1: ec2 user data
\end{frame}
\begin{frame}[label={sec:orgb07bc60},fragile]{NixOS on AWS three ways - \#1}
 \tiny
...
\end{minted}
\end{frame}
\begin{frame}[label={sec:org7e69090},fragile]{NixOS on AWS three ways - \#1}
...
\end{exampleblock}
\end{frame}
\begin{frame}[label={sec:org595c8b6},fragile]{NixOS on AWS three ways - \#1}
...
\end{exampleblock}
\end{frame}
\begin{frame}[label={sec:orgf20fce9},fragile]{NixOS on AWS three ways - \#1}
...
\end{exampleblock}
\end{frame}
\begin{frame}[label={sec:org1eb78cf}]{NixOS on AWS three ways}
\#2: deploy-rs
\end{frame}
\begin{frame}[label={sec:orgcc35783},fragile]{NixOS on AWS three ways - \#2}
 \tiny
\begin{minted}[]{nix}
deploy = {
  nodes = {
...
\end{minted}
\end{frame}
\begin{frame}[label={sec:orgfb975ca},fragile]{NixOS on AWS three ways - \#2}
...
\end{exampleblock}
\end{frame}
\begin{frame}[label={sec:orgd4a075b},fragile]{NixOS on AWS three ways - \#2}
...
\end{exampleblock}
\end{frame}
\begin{frame}[label={sec:orgf32cd14},fragile]{NixOS on AWS three ways - \#2}
...
\end{exampleblock}
\end{frame}
\begin{frame}[label={sec:org9163f03},fragile]{NixOS on AWS three ways - \#2}
...
\end{exampleblock}
\end{frame}
\begin{frame}[label={sec:org5c5057a}]{NixOS on AWS three ways}
\#3: nixos-generators
\end{frame}
\begin{frame}[label={sec:org85f02e4},fragile]{NixOS on AWS three ways \#3}
 \tiny
\begin{minted}[]{nix}

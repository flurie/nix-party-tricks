...
\section{Preamble}
\label{sec:org82c4215}
\begin{frame}[label={sec:org0450f8a}]{Nix is magic}
Surprise, this is a magic show!

Nix is a special kind of magic for specifying pretty much any output you could want.
\end{frame}
\begin{frame}[label={sec:org72c1dd7}]{Nothing up my sleeve\ldots{}}
Behold, a fresh VM.

I am clicking through these things because I am very serious.
\end{frame}
\begin{frame}[label={sec:org749bcc6}]{\ldots{}but cards on the table}
There's always something to install when you start.
\end{frame}
\begin{frame}[label={sec:org805def2}]{For those playing along at home}
...
\label{sec:org6f9ae66}
\begin{frame}[label={sec:orga82fd73}]{Purpose}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org823b73a}]{Let's install nix!}
...
\end{exampleblock}
\end{frame}
\begin{frame}[label={sec:org253de6a},fragile]{And let's install some things to make our lives easier (and this demo shorter)}
...
\label{sec:org4ed341a}
\begin{frame}[label={sec:orgdc16a96}]{Nix the First: Language}
Main features:
\begin{itemize}
\item functional
\item dynamic
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org5f0e0b2}]{Nix the First: Language (con't)}
Quirky type system:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org5914a9f}]{Nix the Second: Package Manager}
nixpkgs
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgeadfeea}]{Nix the Second: Package Manager (con't)}
\begin{figure}[htbp]
\centering
...
\end{figure}
\end{frame}
\begin{frame}[label={sec:org7ab45f3}]{Nix the Third: Linux Distribution}
NixOS
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org11f0e32},fragile]{Nix the Fourth: \emph{misc} tooling}
...
\label{sec:org8306972}
\begin{frame}[label={sec:org857b1c6}]{Misc tools for environment management}
...
\end{alertblock}
\end{frame}
\begin{frame}[label={sec:orgb73eba4}]{Let's install direnv!}
...
\end{exampleblock}
\end{frame}
\begin{frame}[label={sec:orgfddc69f},fragile]{Let's grab the code\ldots{}}
...
\end{exampleblock}
\end{frame}
\begin{frame}[label={sec:org519e265},fragile]{\ldots{}and then let the magic take hold}
...
\end{exampleblock}
\end{frame}
\begin{frame}[label={sec:orge090b70}]{Tour our new powers}
\begin{figure}[htbp]
\centering
...
\end{figure}
\end{frame}
\begin{frame}[label={sec:org13c6a53},fragile]{A note about creds}
...
\end{alertblock}
\end{frame}
\begin{frame}[label={sec:org71d2737},fragile]{Enter AWS with train}
...
\end{exampleblock}
\end{frame}
\begin{frame}[label={sec:org2152748},fragile]{Time to test the thing out}
...
\label{sec:org664c508}
\begin{frame}[label={sec:org0388bee},fragile]{Preamble}
...
\end{exampleblock}
\end{frame}
\begin{frame}[label={sec:orge308a2e}]{Misc tools for deployment management}
\begin{alertblock}{Tools we will use in this section}
\begin{itemize}
\item cachix: arbitrary caching for nix derivations
...
\end{alertblock}
\end{frame}
\begin{frame}[label={sec:orgd352786}]{NixOS on AWS three ways}
\#1: ec2 user data
\end{frame}
\begin{frame}[label={sec:org6d9b47c},fragile]{NixOS on AWS three ways - \#1}
 \tiny
...
\end{minted}
\end{frame}
\begin{frame}[label={sec:org2f77c4a},fragile]{NixOS on AWS three ways - \#1}
...
\end{exampleblock}
\end{frame}
\begin{frame}[label={sec:org05fc1ee},fragile]{NixOS on AWS three ways - \#1}
...
\end{exampleblock}
\end{frame}
\begin{frame}[label={sec:org4522c50},fragile]{NixOS on AWS three ways - \#1}
...
\end{exampleblock}
\end{frame}
\begin{frame}[label={sec:orgbff4bb6}]{NixOS on AWS three ways}
\#2: deploy-rs
\end{frame}
\begin{frame}[label={sec:orge73b377},fragile]{NixOS on AWS three ways - \#2}
 \tiny
\begin{minted}[]{nix}
deploy = {
  nodes = {
...
\end{minted}
\end{frame}
\begin{frame}[label={sec:org0658623},fragile]{NixOS on AWS three ways - \#2}
...
\end{exampleblock}
\end{frame}
\begin{frame}[label={sec:orgca4e16f},fragile]{NixOS on AWS three ways - \#2}
...
\end{exampleblock}
\end{frame}
\begin{frame}[label={sec:orge87f14c},fragile]{NixOS on AWS three ways - \#2}
...
\end{exampleblock}
\end{frame}
\begin{frame}[label={sec:org85047ee},fragile]{NixOS on AWS three ways - \#2}
...
\end{exampleblock}
\end{frame}
\begin{frame}[label={sec:orgaa55599}]{NixOS on AWS three ways}
\#3: nixos-generators
\end{frame}
\begin{frame}[label={sec:orge6f6c9c},fragile]{NixOS on AWS three ways \#3}
 \tiny
\begin{minted}[]{nix}
